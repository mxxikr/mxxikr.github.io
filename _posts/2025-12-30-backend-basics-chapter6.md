---
title: "주니어 백엔드 개발자가 반드시 알아야 할 지식 - 6장"
author:
  name: mxxikr
...
- 애플리케이션 레벨
  ...
  - `Atomic`
    - `AtomicInteger` 등 CAS 연산 활용
  - 동시성 컬렉션
    - `ConcurrentHashMap`, `CopyOnWriteArrayList`
- DB 레벨
  - 선점 잠금
    - `SELECT ... FOR UPDATE` (외부 연동 시 권장)
  - 비선점 잠금
    - version 컬럼으로 충돌 감지 (읽기 성능 우수)

### 해야 할 것

- 잠금은 반드시 `finally`에서 해제함
- DB 업데이트는 원자적 연산 사용함 (`SET count = count + 1`)
- 여러 잠금 획득 시 항상 동일한 순서 유지함

### 하지 말아야 할 것

- 조회 후 애플리케이션에서 증감하지 않음 (`count = count + 1`)
- 서로 다른 순서로 여러 잠금 획득하지 않음 (교착 상태 발생)
...
| 읽기 많음     | ReadWriteLock / 비선점 잠금 |
| 외부 API 연동 | 선점 잠금                   |
| 자원 수 제한  | Semaphore                   |

- 동시성 문제는 테스트에서 발견되지 않아도 운영 환경에서 발생할 수 있으므로 부하 테스트로 반드시 검증함

<br/><br/>

## Reference

- [주니어 백엔드 개발자가 반드시 알아야 할 지식](http://www.yes24.com/Product/Goods/125306759)
