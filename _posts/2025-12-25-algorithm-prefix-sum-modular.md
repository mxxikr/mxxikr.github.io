...
- 나머지 연산은 덧셈과 뺄셈에 대해 닫혀 있으며 연산의 순서를 바꾸어도 결과가 동일함
- 정의
  $$(A + B) \% M = ((A \% M) + (B \% M)) \% M$$
  $$(A - B) \% M = ((A \% M) - (B \% M)) \% M$$
- 의미
...
- 이 성질을 증명하면 복잡한 구간 합 계산을 단순한 나머지 비교로 치환할 수 있음
...
- 임의의 두 인덱스 $x$, $y$에 대하여 누적 합의 나머지($S[x] \% M$)와 누적 합의 나머지($S[y] \% M$)가 같다면 그 사이의 구간 합은 반드시 $M$의 배수임
