...
- 람다식은 매개변수를 갖는 코드 블록으로 익명 클래스의 객체를 생성하는 부분을 수식화한 것임
...
- 메소드 정의에서 반환형 왼편에 각 괄호 <> 안에 타입 매개 변수를 표시
    - 타입 매개 변수를 메소드의 반환형이나 메소드 매개변수의 자료형, 지역 변수의 자료형으로 사용할 수 있음
...
- 람다식 기본 문법
    ...
    - 단, 실행문이 return문 뿐이라면 return과 (수식 다음의) 세미콜론, 중괄호를 동시에 생략 해야 하고 **1개의 수식**만 적어야 함
