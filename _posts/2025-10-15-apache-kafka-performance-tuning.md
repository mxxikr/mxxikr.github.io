...
- **지원 알고리즘 비교**
  ...
  - CPU 사용량은 메시지 크기, 데이터 특성(압축 가능성), 하드웨어 성능에 따라 크게 달라짐
  - 아래 수치는 일반적인 JSON/텍스트 데이터 기준 참고치이며, 실제 환경에서는 벤치마크 테스트 필수

  | 알고리즘 | 압축률          | CPU 영향 (압축 없음 대비) | 처리 속도 | 적합한 경우                   |
  | -------- | --------------- | ------------------------- | --------- | ----------------------------- |
  | gzip     | 매우 높음 (40%) | 3-5배 증가 가능           | 가장 느림 | 장기 보관, 네트워크 제약 심함 |
  | snappy   | 중간 (60%)      | 1.5-2배 증가              | 빠름      | 일반 메시징, 로그 수집        |
  | lz4      | 낮음 (70%)      | 1.2-1.5배 증가            | 매우 빠름 | 실시간 처리 우선              |
  | zstd     | 높음 (45%)      | 2-3배 증가                | 중간      | 압축률/성능 균형              |
