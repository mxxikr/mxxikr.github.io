...
| :---------- | :--------------------------- | :---------- | :-------------------------- |
| **Phase 1** | 언어 X를 다른 언어 Y로 구현  | Y의 개발자  | X가 작동 가능해짐 (Y 필요)  |
...
- Java 코드로 작성된 컴파일러 소스를 Java 컴파일러로 컴파일하여 실행 가능한 Kotlin 컴파일러(바이트코드)를 생성함
- 이 컴파일러는 Kotlin 소스 코드를 입력받아 Kotlin 바이트코드를 출력하는 최소한의 기능을 제공함

### Phase 2 - 첫 번째 자가 호스팅 (2015-2016)

- Kotlin 컴파일러의 소스 코드 자체를 Kotlin으로 다시 작성하는 작업을 진행함
- Java로 만든 원본 컴파일러(J)로 Kotlin으로 작성된 컴파일러(K1)를 컴파일함
- K1으로 K1 자신의 소스 코드를 다시 컴파일하여 K2를 생성하고, 이 과정을 반복하여 점진적으로 개선함
- 이것이 자가 호스팅(Self-Hosting)의 주요 개념이며, Kotlin이 프로덕션급 언어임을 증명하는 전환점이었음

### Phase 3 - K2 컴파일러 (2022-2024)

- JetBrains 주도하에 Google과의 긴밀한 협력을 통해 Kotlin 컴파일러를 처음부터 다시 작성하는 프로젝트를 진행함
- 여전히 Kotlin으로 작성되었으며, Frontend와 Backend의 완전한 분리를 통한 성능 최적화를 목표로 함
- K1은 원본 Kotlin 컴파일러로 현재도 사용되고 있으며, K2는 Kotlin 2.0에서 기본 컴파일러로 채택됨
- K2 컴파일러 도입으로 더 빠른 컴파일 속도, 향상된 IDE 통합, 더 나은 에러 메시지가 가능해짐
...
| :---------- | :--------------- | :-------------------------------- |
| **Java**    | C (Sun, 1995)    | javac는 Java로 작성 (JVM은 C/C++) |
| **GCC (C)** | 이전 버전의 GCC  | 3단계 부트스트래핑                |
| **Python**  | C (CPython)      | PyPy가 Python으로 작성됨          |
...
- **Stage 1**
  - 기존 C 컴파일러로 최소한의 GCC를 만듦
- **Stage 2**
  - Stage 1 결과물(GCC)로 GCC를 다시 컴파일하여 더 많은 기능을 포함한 버전 생성
- **Stage 3**
  - Stage 2 결과물로 GCC를 한 번 더 컴파일하여 최적화되고 검증된 최종 버전 완성
- 각 단계에서 더 나은 컴파일러로 더 나은 컴파일러를 만들도록 검증하고 점진적으로 개선함
...
- Java와의 완벽한 호환
  - Kotlin의 부트스트래핑이 Java에서 시작했기 때문에 지금도 JVM에서 100% 호환됨
  - Java 생태계(Spring, Hibernate 등)를 그대로 활용 가능함
- 계속 진화 중
  - K2 컴파일러 도입으로 Kotlin은 더 빠르고 안정적으로 발전할 예정임

### 부트스트래핑 여부와 언어의 성숙도

- 부트스트랩된 언어 (Kotlin, Java, Go, Python, C, Rust)
  - 이 언어는 자신을 컴파일할 수 있음
  - 복잡한 시스템도 만들 수 있다는 증명
  - 프로덕션 사용이 안전함
- 부트스트랩되지 않은 언어 (신규 실험적 언어들)
  - 아직 다른 언어가 필요함
  - 실험적 단계로 장기 안정성이 미확실함
