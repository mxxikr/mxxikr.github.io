...
- `/root`
  - **관리자(root 사용자) 홈 디렉토리** (예: Windows의 C:\Users\Administrator)
  - ⚠️ 주의: `/` (루트 디렉토리, 최상위)와 혼동하지 말것
- `/sbin`
  - **필수적인 system 명령어 저장소** (예: C:\Windows\wow64)
- `/bin`
  - **필수적인 기본 명령어 저장소** (예: C:\Windows\system32)
- `/boot`
  - **부팅 파일 저장소** (부팅 과정에서 필요한 모든 구성 요소 존재)
- `/home`
  - **사용자 홈 디렉토리 저장소** (예: C\users\사용자명)
- `/usr`
  - **사용자 애플리케이션 및 시스템 리소스 저장소**
  - Unix System Resources의 약자 (실행 파일, 라이브러리, 문서 등)
  - 예: Windows의 C:\Program Files
- `/var`
  - **자주 변경되는 비휘발성 데이터**
