...
2. **날짜는 다르지만 데이터는 동일한 경우**

   - 데이터를 수정해서 날짜가 다르지만, 실제 데이터 결과는 똑같은 경우
...
- **Public과 Private 캐시**
  ...
  - `private`
    - 응답이 해당 사용자만을 위한 것, private 캐시(브라우저 캐시)에 저장 (기본값)
...
- **응답 시간 단축**
  ...
  - 원 서버 직접 접근: 500ms
  - 프록시 캐시 사용: 100ms (5배 개선)

- **캐시 종류**
  - `private`: 브라우저 캐시 (개인 사용자 전용)
  - `public`: 프록시 캐시 (공용 캐시 서버)
...
- **no-cache**
  - 데이터는 캐시하되, 항상 원 서버에 검증 후 사용
- **no-store**
  - 캐시 저장 금지, 메모리에서 사용 후 즉시 삭제 (민감 정보)
- **must-revalidate**
  - 캐시 만료 후 원 서버 검증 필수, 실패 시 504 오류 발생
- **Pragma**
  - HTTP 1.0 하위 호환, `no-cache`와 동일
...
- **차이점 (네트워크 단절 시)**
  - `no-cache`: 프록시 설정에 따라 오래된 캐시 반환 가능
  - `must-revalidate`: 반드시 504 오류 발생
