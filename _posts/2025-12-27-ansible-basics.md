...
- 특징
  - Agentless (에이전트리스)
    ...
    - SSH 접속 권한과 Python 설치가 필요함 (Managed Node 포함)
      - 최신 Ansible은 Python 3.x 사용을 권장함
      - Python이 없는 환경(초경량 컨테이너 등)에서는 `raw` 모듈 사용이 제한적으로 가능함
...
- 단점
  - 속도
    ...
    - `Mitogen` 플러그인 사용이나 `pipelining = True` 설정으로 성능 최적화가 가능함
  - 복잡성
    - 로직이 복잡해질수록 YAML 파일 관리가 어려워질 수 있음
  - OS 지원
    - 리눅스에 비해 윈도우 지원은 상대적으로 부족할 수 있음
...
- Terraform
  - AWS/GCP에 VM, VPC, Security Group 등 하드웨어 인프라를 생성함 (집 짓기)
- Ansible
  - 생성된 VM에 접속하여 Docker 설치 및 환경 설정을 주입함 (인테리어)
- Docker/K8s
  - 실제 애플리케이션 컨테이너를 배포하고 실행함 (입주)
