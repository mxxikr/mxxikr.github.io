...
### 레이아웃 깨짐 2 해결 - 유연한 파싱 로직 도입 (배포 이슈)
- 언어 라벨 텍스트에 포함된 특수문자나 배포 환경의 HTML 최적화로 인한 태그 변형 방지
- **해결**
  - **유연한 파싱 로직 도입**
    - `<div class="highlight">` 전체가 아닌 접두사(`<div class="highlight"`)만으로 분할하여 변형된 태그도 안전하게 감지하도록 개선
  - **태그 수동 재조립**
    - 소실될 수 있는 태그 속성과 닫는 괄호(`>`)를 로직에서 직접 추적하여 재조립해 텍스트 누출(`class="highlight">`) 원천 봉쇄
  - `data-label-text` 속성 주입 시 **`escape` 필터**를 적용하여 특수문자로 인한 구조 깨짐 방지
