...
- 핵심 기능과 부가 기능 분리
  - 포인트 적립, 푸시 알림 등 부가 기능은 비동기로 처리
  - 실패해도 메인 플로우에 영향을 주지 않도록 설계
- 메시징 시스템 도입 검토
  - 시스템 간 직접 호출 대신 카프카 등 메시징 기반 연동 고려
  - 재시도, 모니터링, 데드레터 큐 구성 필요
- 아웃박스 패턴 적용
  - 메시지 전송이 중요한 경우 아웃박스 테이블 사용
  - 트랜잭션 원자성 보장
- 멱등성 확보
  - 메시지 중복 수신에 대비한 멱등성 API 설계
  - 고유 메시지 ID로 중복 처리 방지
- 배치 처리 활용
  - 정산, 대용량 데이터 등은 배치로 처리
  - 실패 시 재실행 가능한 구조로 설계
