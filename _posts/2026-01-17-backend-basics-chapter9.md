...
- 인프라와 플랫폼 관리
  - 서버의 물리적 장비와 클라우드 서비스 운영
- 서비스 개발
  - 인프라와 플랫폼을 기반으로 서비스 로직 구현
- 백엔드 개발
  - OS 설정, 관리, 설정 등 서버 관련 업무
...
- OS 계정 관리
  - root 계정 직접 사용 최소화
  - sudo 설정 시 NOPASSWD 신중히 사용
  - 파일 권한은 최소 권한 원칙 적용
- 프로세스 관리
  - 중요한 서비스는 nohup으로 실행
  - htop으로 주기적으로 프로세스 모니터링
  - 좀비 프로세스 발견 시 즉시 처리
- 디스크 관리
  ...
  - 오래된 로그는 자동 삭제/압축
- 파일 디스크립터
  - 애플리케이션에 맞게 적절한 값으로 설정
  - `/etc/security/limits.conf`에서 영구 설정
  - systemd 서비스는 서비스 파일에서 설정
- 시간 동기화
  - chrony나 ntp 설정
  - 타임스탬프 로깅 시 시간 동기화 확인
- 크론 활용
  - 백업 작업 자동화
  - 로그 정리 자동화
  - DB 정리 작업 스케줄링
...
- 네트워크 진단
  - 서비스 배포 전 포트 충돌 확인
  - 외부 API 연결 시 nc로 연결 테스트
  - netstat으로 현재 연결 상태 확인
