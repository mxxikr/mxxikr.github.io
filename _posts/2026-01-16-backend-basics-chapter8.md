...
- 사용자에게 알리는 것을 넘어 계정 사용 중지와 같은 정책을 적용할 수도 있음
  ...
  - 이를 통해 브루트 포스 공격에 대응함
  - 이메일이나 SMS로 계정 활동 알림 전송

### 브루트 포스(Brute Force) 공격

- 위키피디아에 따르면 브루트 포스 공격은 다음과 같이 정의됨

> 특정한 암호를 풀기 위해 가능한 모든 값을 대입하는 것을 의미한다. 대부분의 암호화 방식은 이론적으로 브루트 포스 공격에 대해 안전하지 못하며, 충분한 시간이 존재한다면 암호화된 정보를 해독할 수 있다.

- 모든 가능한 조합을 시도하는 방식으로 암호를 찾는 것임
...
- 인증 시스템 강화
  ...
  - 토큰 만료 시간 설정
- 암호화 적용
  - 비밀번호는 반드시 BCrypt 등 단방향 암호화 사용
  - Salt 값을 사용자별로 다르게 적용
  - 민감한 개인 정보는 AES로 양방향 암호화
- API 보안 강화
  - 중요한 API는 HMAC으로 무결성 검증
  - HTTPS 통신 필수
  - CORS, CSRF 대응 설정
- 데이터 노출 최소화
  - 백오피스 화면에서 개인 정보 마스킹 처리
  - 고객 목록 조회 권한 제한
  - 비정상 접근 패턴 탐지 로직 구현
- 시큐어 코딩
  - Prepared Statement 사용
  - 입력값 검증 로직 추가
  - 에러 메시지에 시스템 정보 노출 방지
- 감사 로그 구축
  - 중요한 사용자 행위 로깅
  - 로그 위변조 방지 시스템 구축
  - 감사 로그 정기 검토 프로세스 수립
