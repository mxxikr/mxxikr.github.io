...
- **특징**
  - 범위 기반 분할로 시계열 데이터 관리에 최적이며 오래된 파티션 삭제가 용이함
...
- **특징**
  - 명시적 값 목록을 기반으로 하며 지역, 부서 등 불연속적인 카테고리별 분할에 적합함
...
- **특징**
  - 해시 함수를 통해 데이터가 자동으로 균등 분산되어 파티션 간 부하 균형을 맞춤
- **주의점**
  - 파티션 프루닝이 어렵고 부분 범위 쿼리 시 모든 파티션을 스캔하므로 등호(`=`) 검색 워크로드에만 유리함
...
- **특징**
  - 여러 방식을 조합하여 더 세밀하고 복잡한 데이터 관리 요구사항을 충족 가능함
